...
{
    public class CMenu<T>:List<MenuEntry<T>>
    {
        public MenuType Type { get; private set; }

        public CMenu(MenuType type, params MenuEntry<T>[] entries)
        {
            this.AddRange(this);
            Type = type;
            if ((Type == MenuType.UpperLetters || Type == MenuType.LowerLetters) && this.Count > 26)
                throw new ArgumentOutOfRangeException("If 'Letters' is chosen as the MenuType, the this should be equal or less than 26!");
        }
        ...
        public string PrintMenu(char SeparatorChar = '.', char FinalChar = '\n')
        {
            ...
            ushort Numbers = 1;
            foreach (MenuEntry<T> ME in this)
            {
                switch (Type)
                {
        ...
        public bool ExecuteEntry(char Letter, T InputValue)
        {
            if (Type == MenuType.LowerLetters)
            {
                ...
                this[Index].ExecuteEntry(InputValue);
            }
        ...
        public bool ExecuteEntry(int MenuIndex, T InputValue)
        {
            ...
            this[MenuIndex-1].ExecuteEntry(InputValue);
            return true;
        }
        /// <summary>
        ...
        /// <param name="InputValue">The input value</param>
        public void ExecuteEntryWithListIndex(int ListIndex, T InputValue)
        {
            this[ListIndex].ExecuteEntry(InputValue);
        }
    }
}
